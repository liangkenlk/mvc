...
namespace WxPayAPI
{
    ...
    public class WxPayData
    {
        ...
        */
        ...
        public SortedDictionary<string, object> FromXmlNotCheck(string xml)
        {
            if (string.IsNullOrEmpty(xml))
            {
                Log.Error(this.GetType().ToString(), "将空的xml串转换为WxPayData不合法!");
        ...
        */
        ...
        {
            string sign = "jsapi_ticket=" + UserAuth.jsapi_ticket + "&noncestr=" + this.GetValue("nonceStr") + "&timestamp=" + this.GetValue("timestamp") + "&url="+HttpContext.Current.Request.Url.ToString().Split('#')[0];
            ...
            byte[] dataToHash = enc.GetBytes(sign);
